...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        time_t currdate = whenVisitedTable.find(temp);
        if(currdate!=time_t()){
            if(ll.date>currdate){
        ...
        for(it=uniqueURLs.begin();it != uniqueURLs.end();it++){
            addurl = addurl && ((*it) != ll.url);
        }
        if(addurl) uniqueURLs.push_back(ll.url);
...
bool LogfileParser::hasVisited(const string& customer, const string& url) const
{
     ...
     */
   	 ...
   	 if(whenVisitedTable.find(temp)!=time_t()){
        	return true;
    	}
    	else{
        	return false;
                                  ...
                                  const string& url) const
{
	...
	string temp = customer+url;
    	if(hasVisited(customer,url)){
        	time_t time = whenVisitedTable.find(temp);
        	return time;
	    }
	else{
        	return time_t();
