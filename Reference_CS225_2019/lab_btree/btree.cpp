...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
	...
	parent->elements.insert(elem_itr, *(mid_elem_itr++));
   	...
   	while(child->elements.size() > mid_elem_idx) {
        	child->elements.erase(child->elements.begin() + mid_elem_idx);
    	}
    	while(child->children.size() > mid_child_idx) {
        	child->children.erase(child->children.begin() + mid_child_idx);
    	}
}
