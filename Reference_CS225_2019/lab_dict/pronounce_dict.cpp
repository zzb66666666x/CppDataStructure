...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
	...
	std::map<std::string, std::vector<std::string>>::const_iterator t1;
	std::map<std::string, std::vector<std::string>>::const_iterator t2;
	t1 = dict.find(temp1);
	t2 = dict.find(temp2);
	if(t1 == dict.end()) return false;
	if(t2 == dict.end()) return false;
